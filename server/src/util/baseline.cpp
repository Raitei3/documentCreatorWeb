...
static
void peel(cv::Mat &src, cv::Mat &dst, int thickness, int line_gap)
{
    src.copyTo(dst);
...
void Baseline::computeBaselines(cv::Mat &img, std::vector<cv::Vec4i> &lines)
{
    cv::Mat dst;
    img.copyTo(dst);
    ...
    const int density_threshold = (int)(212.453255 * (character_density*character_density) - 306.0151077 * character_density + 244.3721012);
    ...
    cv::HoughLinesP(dst, lines, 1, CV_PI/180, 20, 4*character_height, 5*character_height);
}
...
static
double distance_to_line(cv::Point begin, cv::Point end, cv::Point x)
{
...
int Baseline::getBaseline(cv::Rect r, std::vector<cv::Vec4i> &lines)
{
    cv::Point p = r.br();
    // We create a point in the bottom-middle of the bouding box of the letter
    p.x -= r.width/2;
    p.y -= r.height/2;
    ...
    for (const auto &l : lines) {
        const double d = distance_to_line(cv::Point(l[0], l[1]), cv::Point(l[2], l[3]), p);
    ...
    // We compute the projection of p (charcater center point) on the baseline equation
    assert(closest_line[0]!=closest_line[2]);
    const double slope = (double)(closest_line[1]-closest_line[3])/(closest_line[0]-closest_line[2]);
    int y = (int)(slope*(p.x - closest_line[0]) + closest_line[1]);

    // If the found baseline is too far from the character, we take the bottom as reference
...
std::pair<int, float> Baseline::getCharacterHeight(cv::Mat img)
{
    ...
    cv::Mat symbol = img(cv::Rect((int)img.cols/3, (int)img.rows/3, (int)img.cols/3, (int)img.rows/3));
