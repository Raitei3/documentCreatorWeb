...
void BlurFilter2::calcSolutions(float a, float b, float discr, float &y1, float &y2)
{
  y1 = (-b+sqrt(discr))/(2*a);
  y2 = (-b-sqrt(discr))/(2*a);
}
...
static void fctEllipse(int x, int horizontal, int vertical, float coeff, float &y1, float &y2)
{
  ...
  if (discr > 0) {
    BlurFilter2::calcSolutions(a, b, discr, y1, y2);
    y1 += vertical;
    y2 += vertical;
  }
  else if (discr == 0) {
...
void BlurFilter2::calculFunction(Function fct, int rows, int x, int y, int vertical, int horizontal, float coeff, int &yFunction, int &y2Function)
{
...
bool BlurFilter2::isNearFunction(int x, int y, int rows, Function fct, int horizontal, int vertical, float coeff, int radius)
{
...
static cv::Mat degradateCenter(const cv::Mat &blurredMat, cv::Mat &resultMat, Function function, int horizontal, int vertical, float coeff, int radius)
{
  ...
  for (int y = 0; y < rows; ++y) {
    ...
    for (int x = 0; x < cols; ++x) {
      if (BlurFilter2::isNearFunction(x, y, rows, function, horizontal, vertical, coeff, radius)) //Center
	r[x] = b[x];
      //else
      //r[x] = o[x];
    }
...
bool BlurFilter2::upperThan(Function fct, int rows, int x, int y, int vertical, int horizontal, float coeff)
{
  int yFunction, y2Function = -1;
  calculFunction(fct, rows, x, y, vertical, horizontal, coeff, yFunction, y2Function);
...
static cv::Mat degradateBorder(const cv::Mat &blurredMat, cv::Mat &resultMat, Function function, Area area, int vertical, float coeff, int horizontal)
{
  ...
  if (area == Area::UP) {
    for (int y = 0; y < rows; ++y) {
      ...
      cv::Vec3b *r = resultMat.ptr<cv::Vec3b>(y);
      for (int x = 0; x < cols; ++x) {
	if (BlurFilter2::upperThan(function, rows, x, y, vertical, horizontal, coeff))
	  r[x] = b[x];
      }
    }
  }
...
cv::Mat applyPattern(const cv::Mat &originalMat, const cv::Mat &patternMat, Method method, int intensity)
{
  ...
  cv::Mat blurredMat = blurFilter2(originalMat, method, intensity);
...
QImage BlurFilter2::apply()
{
 QImage finalImg;

 if (_mode == Mode::COMPLETE)
   finalImg  = blurFilter2(_original, _method, _intensity);
 else
...
cv::Mat blurFilter2(const cv::Mat &imgOriginal, Method method, int intensity)
{
...
cv::Mat blurFilter2(const cv::Mat &imgOriginal, Method method, int intensity, Function function, Area area, float coeff, int vertical, int horizontal, int radius)
{
  //B:TODO:OPTIM:
  ...
  cv::Mat matOut = blurFilter2(imgOriginal, method, intensity);
  return degradateArea(imgOriginal, matOut, function, area, coeff, vertical, horizontal, radius);

}

QImage blurFilter2(const QImage &imgOriginal, Method method, int intensity)
{
  cv::Mat matIn = Convertor::getCvMat(imgOriginal);

  cv::Mat matOut = blurFilter2(matIn, method, intensity);
...
QImage blurFilter2(const QImage &imgOriginal, Method method, int intensity, Function function, Area area, float coeff, int vertical, int horizontal, int radius)
{
  cv::Mat matIn = Convertor::getCvMat(imgOriginal);

  cv::Mat matOut = blurFilter2(matIn, method, intensity, function, area, coeff, vertical, horizontal, radius);
