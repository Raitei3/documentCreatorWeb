...
class GrayscaleCharsDegradationModel
{
public:
...
private:
    ...
    void initialize(const cv::Mat &input);
  ...
  //void RapidlyGrayscaleDegradationByTypes();
    ...
    std::vector<float*> getEllipsePoints(cv::Rect &rec,int &max_B, int &min_B, cv::Point pos, double a, double b, double theta);
  //std::vector<cv::Point> getLinePoints(cv::Point A, cv::Point B) const;

    void DegradedLine(float* Line, cv::Point Centre, int Centre_gray, int pixel_signma, int sigma_gaussien,
                      cv::Mat &imgGrayOutput, bool isFtoB);
    void RapidlyDegradeLine(float* Line, cv::Point Centre, int Centre_gray, cv::Mat &imgGrayOutput);
