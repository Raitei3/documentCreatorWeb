...
typedef cv::RNG MyRNG;

struct NormalDistribution
{
  NormalDistribution(MyRNG &rng, double mean, double sigma)
    : m_rng(rng), m_mean(mean), m_sigma(sigma)
  {}
...
GrayscaleCharsDegradationModel::GrayscaleCharsDegradationModel(const cv::Mat &img) :
  _isTest(false),
  _width(0), _height(0),
  _avgBackground(0), _avgForeground(0), _nb_connectedcomponants(0), _MAX_a0(0),
  _alpha(0.0), _beta(0.0), _alpha0(0.0), _beta0(0.0), _nf(0.0), _nb(0.0),
  _a0(0.0f), _g(0.0f), _MAX_Gradient(0.0f), _AVG_Gradient(0.0f), _sigma_gausien(0.0f),
  _is4connected(false),
  _Mean_CC_Distance(0.0f), _Max_CC_Distance(0.0f),
  _Mean_CC_Stroke(0.0f), _Max_CC_Stroke(0.0f),
...
void GrayscaleCharsDegradationModel::initialize(const cv::Mat &imgInput)
{
  ...
  _mat_binary = binarize(); //set _nb_connectedcomponants to NO_CC if no CC found.
  ...
  if (_nb_connectedcomponants != NO_CC) {
    ...
    auto time1 = t1-t0;
    auto time2 = t2-t1;
    std::cerr<<"Time extractConnectedComponantInfos="<<std::chrono::duration<double, std::milli>(time1).count()<<"ms for _nb_connectedcomponants="<<_nb_connectedcomponants<<"\n";
    std::cerr<<"Time calculatePixelsDistanceTransform="<<std::chrono::duration<double, std::milli>(time2).count()<<"ms\n";
...
float GrayscaleCharsDegradationModel::getAvgStrokeWidth() const
{
  return _Mean_CC_Stroke;
}

int GrayscaleCharsDegradationModel::getNumberOfConnectedComponants() const
{
  return _nb_connectedcomponants;
}

cv::Mat GrayscaleCharsDegradationModel::getImageDegraded_cv()
{
  return _mat_output;
}

cv::Mat GrayscaleCharsDegradationModel::getImageGray_cv()
{
  return _mat_gray;
}

cv::Mat GrayscaleCharsDegradationModel::degradateByLevel_cv(int level)
{
  cv::Mat src;
  if (level <= 4) {
...
cv::Mat GrayscaleCharsDegradationModel::degradate_cv(int level, float I, float O, float D)
{
  ...
  _nbSPs_User = NUMBER_NOISE_PER_CC*getNumberOfConnectedComponants()*level/5;
...
#ifdef TIMING
  {
    ...
    auto time5 = t5-t4;
    auto time6 = t6-t5;
    // qDebug()<<"time calculatePixelsProbability="<<std::chrono::duration<double, std::milli>(time1).count()<<"ms\n";
    // qDebug()<<"time flipPixelsByProbability="<<std::chrono::duration<double, std::milli>(time2).count()<<"ms\n";
    // qDebug()<<"time calculateNoiseRegionType="<<std::chrono::duration<double, std::milli>(time3).count()<<"ms\n";
    // qDebug()<<"time separateSeedPointsByTypes="<<std::chrono::duration<double, std::milli>(time4).count()<<"ms\n";
    // qDebug()<<"time assignmentSizeOfNoiseRegion="<<std::chrono::duration<double, std::milli>(time5).count()<<"ms\n";
    // qDebug()<<"time grayscaleDegradationByTypes="<<std::chrono::duration<double, std::milli>(time6).count()<<"ms\n";
  }
#endif
 ...
 */
void GrayscaleCharsDegradationModel::calculatePixelsDistanceTransform()
{
  assert(_mat_reduce_pixels.type() == CV_8UC1);
  assert(_mat_reduce_pixels.rows == _height && _mat_reduce_pixels.cols == _width);
  ...
  for (int y=0; y<height; ++y) {
	...
	if (pixel.distanceToEdge > 0) {
	  ...
	  if (pixel.pos.x > 0 && pixel.pos.x < (width-1) 
	      && pixel.pos.y > 0 &&  pixel.pos.y < (height - 1)) {
	    
	    const float dx = (_mat_gray.at<uchar>(pixel.pos.y, pixel.pos.x+1) - _mat_gray.at<uchar>(pixel.pos.y, pixel.pos.x-1))/2;
	    const float dy = (_mat_gray.at<uchar>(pixel.pos.y+1, pixel.pos.x) - _mat_gray.at<uchar>(pixel.pos.y-1, pixel.pos.x))/2;
	  ...
	  if (pixel.gradient_angle == 0) 
	    pixel.gradient_angle = 10;  //B: why ??? why this value ???
...
int GrayscaleCharsDegradationModel::getMinStrokeWidthAtASeedPoint(const Seedpoint &sp, int &angle, cv::Point &A, cv::Point &B) const
{
  //B: some code duplication with calculateApproximatelyDistanceFromBord() ???

  assert(_mat_CCs.rows == _height && _mat_CCs.cols == _width && _mat_CCs.type() == CV_8UC1);
...
static
std::vector<cv::Point> getLinePoints(cv::Point pt1, cv::Point pt2, int width, int height)
{
...
void GrayscaleCharsDegradationModel::calculateNoiseRegionType()
{
  ...
  for (Seedpoint& sp : _listSeedPoints) {
	...
	pB2 = listOut[2];
      ...
      for (int x=ax; x<=bx; ++x) 
	listPointsInLine.push_back(cv::Point(x, yo));

      ++dbg2;
    }
    else if (gradient_angle==90) {// tan = infinit
      ...
      listPointsInLine.reserve(nbPts);
      for (int y=dy; y<=cy; ++y) 
	listPointsInLine.push_back(cv::Point(xo, y));
    ...
    for (const cv::Point p : listEdgePoints) {

      const float dis = sqrt( (xo - p.x)*(xo - p.x) + (yo - p.y)*(yo - p.y)); //B:TODO:OPTIM: avoid sqrt
      if (first_min > dis) {
...
void GrayscaleCharsDegradationModel::assignmentSizeOfNoiseRegion() 
{
 ...
 */
void GrayscaleCharsDegradationModel::extractConnectedComponantInfos()
{
  assert(_mat_binary.type() == CV_8UC1);

  CCs ccsInfo;
  const int connectivity = 4;
  ConnectedComponentExtraction::extractAllConnectedComponents(_mat_binary, ccsInfo, connectivity);


  _mat_reduce_pixels = cv::Mat::zeros(_mat_binary.size(), _mat_binary.type());
  _mat_CCs = cv::Mat(_mat_binary.size(), _mat_binary.type());

  cv::Mat mat_Accepted_CCs = cv::Mat::zeros(_mat_binary.size(), _mat_binary.type());
  ...
  //B: This does not work if we have a small image and a big text.
  {
    size_t m0 = 0;
  ...
  for (const CC &ccItem : ccsInfo) {
    ...
    if (ccItem.size() >= minCCSize) { //remove small CCs
      ...
      for (size_t i=0; i<ccItem.size(); ++i) {
	...
	if (min_y > y)
      ...
      if (dis_square <= max_dis_square) { //remove big CCs
	...
	// (thus if several CC bounding boxes intersect, we count their pixels several times)
	// [In the original version we could also go outside the CC bounding box]
	...
	//which version is correct ??? [it should change only _Mean_CC_Stroke]
	{
	  ...
	  width_of_CC = cc.size();
	  counter = 1;
	  for (size_t i=1; i<cc.size(); ++i) { //start from 1
	    const int x = cc[i].x;
	    const int y = cc[i].y;
	    if (y == prev_y) {
	      if (x > prev_x+1) {
  ...
  findContours( mat_Accepted_CCs, _contours, hierarchy, cv::RETR_TREE, cv::CHAIN_APPROX_NONE, cv::Point(0, 0) );
...
void GrayscaleCharsDegradationModel::separateSeedPointsByTypes(float percent_cheval, float percent_diffusion)
{
  ...
  /**/
  size_t nbDiffusion = static_cast<size_t>(percent_diffusion*_nbSPs_User*0.01f);
  ...
  //// Independent spot
  // All most of background pixels have smallest distance and subtraction of two thresholds
  // Therefore, we choose a half of background pixels and another of foreground pixels.
  listFirstThreslhold.clear();
  ...
  if ((nbIndependent/2) < listFirstThreslhold.size() && ! listFirstThreslhold.empty()) {

    std::sort(listFirstThreslhold.begin(), listFirstThreslhold.end());

    assert(nbIndependent/2 < listFirstThreslhold.size());
    const float thres = listFirstThreslhold[listFirstThreslhold.size() - nbIndependent/2];
  ...
  counter = 0;
  if ((nbIndependent - selectedBGSPs) < listSecondThreslhold.size() && ! listSecondThreslhold.empty()) {

    sort(listSecondThreslhold.begin(), listSecondThreslhold.end());

    assert((nbIndependent - selectedBGSPs) < listSecondThreslhold.size());
    const float thres = listSecondThreslhold[listSecondThreslhold.size() - (nbIndependent - selectedBGSPs)];
  ...
  if ((selectedBGSPs + counter) < nbIndependent) {
	...
	nbNeedMore = listFirstThreslhold.size();
      ...
      const float thres = listFirstThreslhold[listFirstThreslhold.size() - nbNeedMore];
...
void GrayscaleCharsDegradationModel::grayscaleDegradationByTypes()
{
  ...
  for (Seedpoint& sp : _listSeedPoints) {
    ...
    while (_g<=(mean_g-sigma_g) || _g>=(mean_g+sigma_g) || _g <0 || _g>1 ) { //B ???
      _g = var_nor_g(); 
      ++limite_g;
      if (limite_g > 500) {
	_g=0.5;
    ...
    if (!sp.pixel.isBackground) {
      ...
      // The difference between generated number and MEAN is not upper than SIGMA.
      int limiter = 0;
      while (std::max(abs(C_gris - min_B), abs(C_gris - max_B)) > _MAX_Gradient && C_gris > (_avgBackground - 15)) {
	...
	for (float* line : lines) {
	  DegradedLine(line, sp.pixel.pos, C_gris, 10, _sigma_gausien, _mat_output, true);
	}
      }
    }
    // background to foreground
      ...
      // The difference between generated number and MEAN is not upper than SIGMA.
      int limiter = 0;
      while (std::max(abs(C_gris - min_B), abs(C_gris - max_B)) > _MAX_Gradient) {
	...
	for (float* line : lines) {
	  DegradedLine(line, sp.pixel.pos, C_gris, 10, _sigma_gausien, _mat_output, false);
	}
      }
  ...
  for (const cv::Rect &r : listRecs) {
    if (r.x >= 0 && r.y >= 0 
	&& (r.x + r.width) <_width && (r.y + r.height) <_height 
	&& r.width >= mask && r.height >= mask) {
	...
	for (int x=0; x<r.width; ++x) {
	  assert((r.y+y)<_height && (r.y + y) >=0 && (r.x + x) < _width && (r.x + x)>=0);
	  dst[r.x +x] = src[x];
	}
      }
  ...
  // If we have overlapping rectangles, the same pixels may be blurred several times...


  listRecs.clear();
}

void GrayscaleCharsDegradationModel::RapidlyDegradeLine(float* Line, cv::Point Centre,
                                                        int Centre_gray,
                                                        cv::Mat &imgGrayOutput)
{
  ...
  if (deltaDis > 0) {
    ...
    while (id < n) {
      ...
      const double dis = sqrt( (B1.x-Centre.x)*(B1.x-Centre.x) + (B1.y -Centre.y)*(B1.y -Centre.y) );
      imgGrayOutput.at<uchar>(B1.y, B1.x) = Centre_gray + (B_gris - Centre_gray)*dis/deltaDis;
    }
  }
}

void GrayscaleCharsDegradationModel::DegradedLine(float* Line, cv::Point Centre,
                                                  int Centre_gray, int pixel_signma, int sigma_gaussien,
                                                  cv::Mat &imgGrayOutput, bool isFtoB)
{
  ...
  if (deltaDis > 0) {
    ...
    while (id < n) {
      ...
      const float dis = sqrtf( (B1.x-Centre.x)*(B1.x-Centre.x) + (B1.y -Centre.y)*(B1.y -Centre.y) );
	...
	if (Centre_gray == 0) 
	  Centre_gray = 1; 
	B1_moyen_gris = /*C_gris + (B_gris - C_gris)*dis/deltaDis;}*/exp((log(B_gris)-log(Centre_gray))*(dis/deltaDis) + log(Centre_gray));
      }//C_gris + (B_gris - C_gris)*dis/deltaDis;

      //NUMBER GENERATOR : normal distribution
      NormalDistribution var_nor_ForPixelEcllipse(rng, B1_moyen_gris, pixel_signma);

      int grisGenerator = var_nor_ForPixelEcllipse();

      if (isFtoB)
	while (grisGenerator < (B1_moyen_gris - pixel_signma) || grisGenerator > (B1_moyen_gris + sigma_gaussien)) {
	...
	while (grisGenerator < 0 || grisGenerator < (B1_moyen_gris - pixel_signma) || grisGenerator > (B1_moyen_gris + pixel_signma)) {
	  grisGenerator = var_nor_ForPixelEcllipse();
	  ++limiter1;
	  if (limiter1 > 100)
	    break;
	...
	grisGenerator = WHITE;
      ...
      assert(B1.y>=0 && B1.y<imgGrayOutput.rows && B1.x>=0 && B1.x<imgGrayOutput.cols);//B
 ...
 */
int GrayscaleCharsDegradationModel::calculateApproximatelyDistanceFromBord(const Pixel &pixel) const
{
  int x = pixel.pos.x;
  int y = pixel.pos.y;
  ...
  for (int i = x; i < x1; ++i) {
    ...
    if (j >= y0) {
      assert(i <_width);
      assert(j>=0 && j<_mat_contour.rows && i>=0 && i<_mat_contour.cols);
...
int GrayscaleCharsDegradationModel::calculateDistanceFromBord(const Pixel &pixel) const
{
  ...
  for (int y=y0; y<y1; ++y) {
    ...
    for (int x=x0; x<x1; ++x) {
      ...
      if (m[x] == 255) {
	const float distSq = (pixel.pos.x - x)*(pixel.pos.x - x) + (pixel.pos.y - y)*(pixel.pos.y - y);
	if (distSq < minDistSq)
	  minDistSq = distSq;
      }
    }
...
std::vector<float*> GrayscaleCharsDegradationModel::getEllipsePoints(cv::Rect &rec, int &max_B, int &min_B,
								     cv::Point centre, 
								     double semi_minor_axis, double semi_major_axis, double theta)
{
  ...
  cv::Mat box = cv::Mat::zeros(cv::Size(_width+ex, _height+ex), CV_8UC1); //B: TOO BIG ??????
  cv::Point newCentre(centre.x+ex/2, centre.y+ex/2);

  cv::ellipse(box, newCentre, cv::Size(semi_major_axis, semi_minor_axis), theta, 0, 360, cv::Scalar(WHITE, WHITE, WHITE), -1);

  const int localZone2 = 2*_local_zone;
  const int x = (newCentre.x - localZone2)>=0 ? (newCentre.x - localZone2) : (0);
  const int y = (newCentre.y - localZone2)>=0 ? (newCentre.y - localZone2) : (0);

  const int wo = (newCentre.x - localZone2)>=0 ? (localZone2) : (newCentre.x);
  const int ho = (newCentre.y - localZone2)>=0 ? (localZone2) : (newCentre.y);
  const int w = wo + ( (newCentre.x + localZone2)< box.cols ? (localZone2) : (box.cols - newCentre.x) );
  const int h = ho + ( (newCentre.y + localZone2)< box.rows ? (localZone2) : (box.rows - newCentre.y) );
  ...
  for (cv::Point p : v) {
	...
	for (int x = x0; x < x1; ++x) {
	  ...
	  if (b[x] == scalar) {
	    const int xt = x - ex/2;
	    assert(yt>=0 && yt<_mat_gray.rows && xt>=0 && xt<_mat_gray.cols);  //B: added 05/2015: to avoid point outside image
	    pointInLine.push_back(cv::Point(xt, yt));
	  }
	}
      }
	...
	pt.x = 0;
      ...
      assert(pt.y >= 0 && pt.y < _mat_gray.rows 
	     && pt.x >= 0 && pt.x < _mat_gray.cols);
	...
	min_B = gray;
      ...
      line[id] = sqrt((p.x - newCentre.x)*(p.x - newCentre.x) + (p.y - newCentre.y)*(p.y - newCentre.y));
      ++id;//1
      line[id] = gray;
      ++id;//2
      for (cv::Point px : pointInLine) {

	assert(px.y>=0 && px.y<_mat_gray.rows && px.x>=0 && px.x<_mat_gray.cols);//B
 ...
 */
cv::Mat GrayscaleCharsDegradationModel::binarize()
{
  cv::Mat img_bin;
  ...
  if (nbPixelForeground > 0 && nbPixelBackground > 0) {
    _avgBackground = totalBackgroundGrayValue/(float)nbPixelBackground;
    _avgForeground = totalForegroundGrayValue/(float)nbPixelForeground;
  }
  else {
 ...
 */
void GrayscaleCharsDegradationModel::calculatePixelsProbability() 
{
  MyRNG rng((unsigned int) time(nullptr));
  ...
  const std::vector<Pixel>::iterator itEnd = _listPixels.end();
  for (std::vector<Pixel>::iterator it = _listPixels.begin();
       it != itEnd; ++it) {
    Pixel &pixel = *it;
...
void GrayscaleCharsDegradationModel::flipPixelsByProbability(float percent_independent)
{
  ...
  const size_t nbSps_Over = _nbSPs_User + _nbSPs_User*percent_independent/100;
  ...
  _listSeedPoints.reserve(std::max(sz/100, (size_t)_height)); //arbitrary
  ...
  for (const Pixel &pixel : _listPixels) {
    if (pixel.probability >= flip_random && pixel.probability <= 1) {
      ...
      //++countBF;
      _listSeedPoints.push_back(Seedpoint(pixel)); //TODO:C++11: emplace_back()
    }
  }
