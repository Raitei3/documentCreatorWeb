 ...
 */
extern  QImage blurFilter2(const QImage &imgOriginal, Method method, int intensity);

/*
  Apply blur to specific image area
*/
extern  QImage blurFilter2(const QImage &imgOriginal, Method method, int intensity, Function function, Area area=Area::UP, float coeff=1, int vertical=0, int horizontal=0, int radius=0); //Vertical : Position vertical of the area, Horizontal : Position Horizontal of the area
...
/*
  Apply blur to whole image
*/
extern  cv::Mat blurFilter2(const cv::Mat &imgOriginal, Method method, int intensity);

/*
  Apply blur to specific image area
*/
extern  cv::Mat blurFilter2(const cv::Mat &imgOriginal, Method method, int intensity, Function function, Area area=Area::UP, float coeff=1, int vertical=0, int horizontal=0, int radius=0);
...
public :
  ...
  explicit BlurFilter2(const QImage &original, Method method, int intensity, Mode mode=Mode::COMPLETE, Function function=Function::LINEAR, Area area=Area::UP, float coeff=1, int vertical=0, int horizontal=0, int radius=0, const QImage &pattern = QImage(), QObject *parent =0) :
     _intensity(intensity), _method(method), _mode(mode), _function(function), _area(area), _coeff(coeff), _vertical(vertical), _horizontal(horizontal), _radius(radius), _original(original), _pattern(pattern)
{}
