...
LineDetectionDialog::LineDetectionDialog(QObject *parent) :
  QObject(parent),
  _thumbnail(nullptr),
...
void LineDetectionDialog::setOriginalImage(const QImage &img)
{
    _originalImg = img;
    process();
...
int LineDetectionDialog::getCharacterHeight(cv::Mat img)
{
...
void LineDetectionDialog::drawBaseLines(const cv::Mat &img, cv::Mat &dst, const std::vector<cv::Vec4i> &lines)
{
    // Smooth the image to remove unwanted noise from extracted CCs
    cv::Mat tmp;
    // Smooth the image to remove unwanted noise from extracted CCs
    ...
    for (const auto &contour : contours) {
        ...
        for (cv::Vec4i l : lines) { //int line=0; line < (int)lines.size(); ++line) {
...
void LineDetectionDialog::process()
{
...
/*
    ...
    _originalLabel->setPixmap(QPixmap::fromImage(blurred.scaled(IMG_WIDTH,IMG_HEIGHT, Qt::KeepAspectRatio, Qt::FastTransformation)));
    */ 
    ...
    cv::HoughLinesP(dst, lines, 1, CV_PI/180, 20, 4*_character_height, 5*_character_height);
