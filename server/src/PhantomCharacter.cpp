...
//static const int NB_TRY_FIT = 5;//Define how many times we will search a character where we can copy the pattern changing the size of pattern

static const int MIN_MARGIN = 1;//Minimum spacing between a character and the degradation we paste
static const int MAX_MARGIN = 2;//Maximum spacing between a character and the degradation we paste

static const float COEFF_MIN_HEIGHT = 0.9; //Coeff with which the height of pattern will be multiplied to get the minimum height of degradation
static const float COEFF_MAX_HEIGHT = 1.05; //Coeff with which the height of pattern will be multiplied to get the minimum height of degradation
static const float COEFF_HEIGHT_MARGIN = 0.35; //Coeff with which the height of character will be multiplied. Added to the height on each side to get the height on which we want to search neighbor (to not search just in the height, but a little more)

static const int MIN_WIDTH = 5;//Minimum of width that the pattern can take (considering also the marge)

static const int MIN_WIDTH_PRC = 10;//Percentage of character's width that the minimum width of pattern will take
static const int MAX_WIDTH_PRC = 30;//Percentage of character's width that the maximum width of pattern will take
static const int MIN_HEIGHT = 5; //Minimum of height that the pattern can take
...
/**
   Compute (axis-aligned) bounding box of connected componant @a cc.

   Return minX==INT_MAX && minY==INT_MAX if @a cc is empty. 
 */
static
void computeRect(const CC &cc, int &minX, int &maxX, int &minY, int &maxY)
{
...
static
void computeSize(const CC &cc, int &width, int &height)
{
 ...
 */
static
int getDistanceBorder(const cv::Mat &pattern, int xOrigin, int yOrigin)
{
  int minDist = INT_MAX;
...
static cv::Vec3b makeGradient(cv::Vec3b oldPixel, cv::Vec3b newPixel, int distance)
{
  if (distance > 1)
    return newPixel;
  ...
  cv::Vec3b resPixel;
  resPixel[0] = cv::saturate_cast<uchar>(alpha * oldPixel[0] + oneMinusAlpha * newPixel[0]);
  resPixel[1] = cv::saturate_cast<uchar>(alpha * oldPixel[1] + oneMinusAlpha * newPixel[1]);
  resPixel[2] = cv::saturate_cast<uchar>(alpha * oldPixel[2] + oneMinusAlpha * newPixel[2]);
...
/**
   Check if connected componant @a charToTest from binary image @a matBin contains the pattern @a matPattern.
 ...
 */
static
cv::Point contains(const cv::Mat &matBin, 
		   const CC &/*charToTest*/, int minX, int maxX, int minY, int maxY,
 ...
 */
static cv::Mat binarize(const cv::Mat &mat, double threshold = 127)
{
  cv::Mat mat_gray;
...
static int getXNearestNeighbor(cv::Mat &outputBin, int xOrigin, int yOrigin, int height, bool leftNeighbor)
{
 ...
 */
void copyTo(const cv::Mat &src, cv::Mat &dst, int origin_x, int origin_y)
{
...
{
  const int r = static_cast<int>(minV + (static_cast<float>(rand()) / RAND_MAX * (maxV - minV + 1)));
		...
		)
{
  ...
  assert(srcPoint.x+pattern.cols <= output.cols && srcPoint.y+pattern.rows <= output.rows);
  ...
  for (int y = 0; y < p_h; ++y) {
    ...
    for (int x = 0; x < p_w; ++x) {
      ...
      if (p[x] == BLACK) {
	const cv::Vec3b oldPixel = d[dstPoint.x+x]; //output.at<cv::Vec3b>(y+dstPoint.y, x+dstPoint.x)
	const cv::Vec3b newPixel = s[srcPoint.x+x]; //output.at<cv::Vec3b>(y+srcPoint.y, x+srcPoint.x)
	
	const uchar oldGray = (oldPixel[0] + oldPixel[1] + oldPixel[2])/3;
	const uchar newGray = (newPixel[0] + newPixel[1] + newPixel[2])/3;
	if (oldGray > newGray) {
	  d[dstPoint.x+x] = makeGradient(oldPixel, newPixel, getDistanceBorder(pattern, x, y));
#ifdef SAVE_DEGRADATIONS_IMAGE
 ...
 * @param[in,out] output
 */
static bool degradeComposant(cv::Mat &output, const CC &degradedCC, const CCs &ccs, int posDegraded
#ifdef SAVE_DEGRADATIONS_IMAGE
			     , cv::Mat &degrads
#endif //SAVE_DEGRADATIONS_IMAGE
			     ) 
{
  ...
  const int minXRight = getXNearestNeighbor(outputBin, maxX, minY, height, false);
  ...
  for (int side = firstSide; side <= lastSide; ++side) { //0 = left, 1 = right
	...
	maxWidth = minWidth+1;
      ...
      widthPattern = boundedRand(minWidth, maxWidth);//rand()% (maxWidth - minWidth) + minWidth ;
	    
      minHeight = height * COEFF_MIN_HEIGHT;
      maxHeight = height * COEFF_MAX_HEIGHT;

      heightPattern = boundedRand(minHeight, maxHeight);//rand()% (maxHeight - minHeight) + minHeight;
	...
	tmp.save("/home/doctorant/testPattern.png");*/
      ...
      if (origin != INVALID_POINT) { //If we found a place where copy, we can apply
	...
	if (side == 0) { //LEFT
	  ...
	  if (leftCharFound)
	    xMinPattern = ((minX + maxXLeft)/2); //The center between the character and his neighbor
	  else
	    xMinPattern = minX - MAX_MARGIN; //if there is not neighbor, we take a fixed maximum width between character and degradation
		  
	  if (xMinPattern < 0)
	    xMinPattern = 0; //to not go out the document

	  xMaxPattern = minX - MIN_MARGIN; // we take a fixed minimum width between character and its degradation
	      
	  if (xMinPattern >= xMaxPattern)
	    xMaxPattern = xMinPattern+1; //to be sure that max > min (+1 to shift toward right and don't go out the document)
	}	    
	else if (side == 1) { //RIGHT
		
	  if (rightCharFound)
	    xMaxPattern = ((maxX + minXRight)/2) - (patternMat.cols/2); //The center between the character and his neighbor (and shift to be symetric with the left degradation)
	  else
	    xMaxPattern = maxX + MAX_MARGIN - patternMat.cols; //if there is not neighbor, we take a fixed maximum (and shift to be symetric with the left degradation)
		
	  if (xMaxPattern > output.cols)
	    xMaxPattern = output.cols; //to not go out the document
		
	  xMinPattern = maxX + MIN_MARGIN;  // we take a fixed minimum width between character and its degradation
		
	  if (xMinPattern >= xMaxPattern)
	    xMinPattern = xMaxPattern-1; //to be sure that max > min (-1 to shift toward left and don't go out the document)

	}

	const int xPattern =  boundedRand(xMinPattern, xMaxPattern);//(int) (rand()%(xMaxPattern - xMinPattern) +  xMinPattern); //random
	      

	transferPattern(patternMat, cv::Point(xPattern, yPattern), origin, output
#ifdef SAVE_DEGRADATIONS_IMAGE
			, degrads
#endif //SAVE_DEGRADATIONS_IMAGE
			);
	...
	//APPLY BLUR
	widthPattern = std::min(widthPattern, output.cols - xPattern); //to be sure to not copy outside the mat
...
cv::Mat phantomCharacter(const cv::Mat &input, int frequency)
{
  ...
  for (size_t i=0; i<sz; ++i) {
    ...
    const size_t areaAABB = width*height;
...
#ifdef SAVE_DEGRADATIONS_IMAGE
  cv::Mat degrads(input.rows, input.cols, input.type(), cv::Scalar(255, 255, 255));  
#endif //SAVE_DEGRADATIONS_IMAGE
  ...
  for (int unsigned i=0; i < ccs.size(); ++i) {
    ...
    if (chosen < probOccurence){ // if random number is included in the x % (x which was chosen by the user)
      chosen = 1;
    }
    
    if (chosen == 1){
      degradeComposant(output, ccs[i], ccs, i
#ifdef SAVE_DEGRADATIONS_IMAGE
		       , degrads
#endif //SAVE_DEGRADATIONS_IMAGE
		       );
