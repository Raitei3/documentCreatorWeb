...
static
std::vector<uchar> getGray(const QImage &psrc)
{
  QImage src = psrc;
  ...
  if (depth == 32) {
    ...
    std::cerr<<"  format="<<(int)src.format()<<"\n";
    std::cerr<<"  Format_Mono="<<(int)QImage::Format_Mono<<"\n";
    std::cerr<<"  Format_Indexed8="<<(int)QImage::Format_Indexed8<<"\n";
    assert(false);
...
BleedThroughDiffusionThreadA::BleedThroughDiffusionThreadA(const std::vector<uchar> &originalRectoGray,
							   const std::vector<uchar> &rectoGray,
							   const std::vector<uchar> &versoGray,
							   int width, int height,
							   QPoint xy, 
...
void BleedThroughDiffusionThreadA::getRecto(std::vector<uchar> &outGray) const
{
  assert(outGray.size() == (size_t)(_width*_height));

  const uchar *s = &_outGray[0];
  uchar *d = &outGray[_y*_width+_x];
...
void BleedThroughDiffusionThreadA::run()  //bleedThrough4b
{
  const int width = _width;
  const int height = _height;
  const int shift = _shift;

  const size_t start = _y*width+_x;
...
static
QImage
bleedThroughMT0(const QImage &originalRecto, const QImage &imgRecto, const QImage &imgVerso, int nbIter, int nbThreadsPerRow=4)
{
...
static
void
copyImagePart(QImage &dst, int x0_dst, int y0_dst, const QImage &src, int x0_src, int y0_src, int w, int h)
{
  assert(dst.depth() >= 8);
  assert(src.depth() >= 8);
...
QImage
bleedThrough_aux(const QImage &originalRecto, const QImage &imgRecto, const QImage &imgVersoP, int nbIter, int x, int y, int nbThreads)
{
  assert(originalRecto.depth() >= 8);
  assert(imgRecto.depth() >= 8);
  assert(imgVersoP.depth() >= 8);

  QImage imgVerso = getOverlappingPart(imgRecto.width(), imgRecto.height(), imgVersoP, x, y);
...
QImage
bleedThrough(const QImage &originalRectoP, const QImage &imgRectoP, const QImage &imgVersoP, int nbIter, int x, int y, int nbThreads)
{
  assert(originalRectoP.size() == imgRectoP.size());
  
  QImage originalRecto = originalRectoP;
  QImage imgRecto = imgRectoP;
  QImage imgVerso = imgVersoP;
  convertToAtLeast8Bit(originalRecto);
  convertToAtLeast8Bit(imgRecto);
  convertToAtLeast8Bit(imgVerso);

  return bleedThrough_aux(originalRecto, imgRecto, imgVerso, nbIter, x, y, nbThreads);
}

QImage
bleedThrough(const QImage &imgRectoP, const QImage &imgVersoP, int nbIter, int x, int y, int nbThreads)
{
  QImage imgRecto = imgRectoP;
  QImage imgVerso = imgVersoP;
  convertToAtLeast8Bit(imgRecto);
  ...
  return bleedThrough_aux(imgRecto.copy(), imgRecto, imgVerso, nbIter, x, y, nbThreads);
}
