...
cv::Mat TextInpainting::getBackground(const cv::Mat &img, const cv::Mat &img_bin, float max_text_area, float max_text_width, float max_text_height)
{
    cv::Mat background;
    ...
    // We consider too big connected components as images, they are then removed from the mask which is to be inpainted
    std::vector<std::vector<cv::Point> > contours;
    cv::findContours(bin.clone(), contours, CV_RETR_CCOMP, CV_CHAIN_APPROX_TC89_KCOS);
    ...
    for (size_t i=0; i<contours.size(); ++i){
      ...
      if (r.width > img_bin.cols * max_text_width || r.height > img_bin.rows * max_text_height
...
cv::Mat TextInpainting::getDistanceMap(const cv::Mat &img, const cv::Mat &img_bin, float char_height){
    ...
    char_height /= 2;
    cv::Mat element = cv::getStructuringElement(2,
