...
void Binarization::preProcess(const cv::Mat &src, cv::Mat &dst, int erosion) {
...
void Binarization::postProcess(const cv::Mat &src, cv::Mat &dst, double v, int win) {
...
void Binarization::shrinkFilter(const cv::Mat &src, cv::Mat &dst, double v, int win) {
...
void Binarization::connectivityFilter(const cv::Mat &src, cv::Mat &dst) {
...
void Binarization::swellFilter(const cv::Mat &src, cv::Mat &dst, int win) {
...
double Binarization::binarize(const cv::Mat &src, cv::Mat &dst, int method, int thresholdType, int blockSize) {
...
void Binarization::applyErosion(const cv::Mat &src, cv::Mat &dst, int erosion_type, int erosion_size) {
...
void Binarization::applyDilation(const cv::Mat &src, cv::Mat &dst, int dilation_type, int dilation_size) {
...
double Binarization::calcLocalStats(const cv::Mat &im, cv::Mat &map_m, cv::Mat &map_s, int winx, int winy) {
  ...
  for (int j = y_firstth ; j<=y_lastth; j++) {   
    ...
    double sum = im_sum.at<double>(j-wyh+winy, winx) - im_sum.at<double>(j-wyh, winx) - im_sum.at<double>(j-wyh+winy, 0) + im_sum.at<double>(j-wyh, 0);
    double sum_sq = im_sum_sq.at<double>(j-wyh+winy, winx) - im_sum_sq.at<double>(j-wyh, winx) - im_sum_sq.at<double>(j-wyh+winy, 0) + im_sum_sq.at<double>(j-wyh, 0);

    double m  = sum / winarea;
    double s  = sqrt ((sum_sq - m*sum)/winarea);
    if (s > max_s)
      max_s = s;
...
void Binarization::NiblackSauvolaWolfJolion(const cv::Mat &imOrig,
					    cv::Mat &output,
					    NiblackVersion version, double dR,
  ...
  const int x_lastth = std::max(im.cols-wxh-1, 0);
