...
static
void 
updateShadowRGB(cv::Vec4b intensityColor, float coeff, 
...
isInMarge(const cv::Mat &matPattern, int x, int y, int width)
{
  ...
  if ( matPattern.at<cv::Vec3b>(y, x) == PIXEL_BLACK) { //not in marge if it's not in black pattern
    
    for (int i = 0; i < width; ++i) {
      if (x-i > 0 && matPattern.at<cv::Vec3b>(y, x-i) != PIXEL_BLACK)
	return true;
      if (x+i < matPattern.cols && matPattern.at<cv::Vec3b>(y, x+i) != PIXEL_BLACK)
	return true;
      if (y-i > 0 && matPattern.at<cv::Vec3b>(y-i, x) != PIXEL_BLACK)
	return true;
      if (y+i < matPattern.rows && matPattern.at<cv::Vec3b>(y+i, x) != PIXEL_BLACK)
	return true;
    }
  }
...
static
void 
drawBorder(cv::Mat &matOut, const cv::Mat &matPattern, int x, int y, int xOrigin, int yOrigin, int width, float intensity)
{  
  ...
  if (isInMarge(matPattern, x, y, width)) {
    const cv::Vec4b intensityColor = matOut.at<cv::Vec4b>(y+yOrigin, x+xOrigin);
    
    unsigned char blue, green, red;
    updateShadowRGB(intensityColor, coeff, blue, green, red);
    const unsigned char alpha = intensityColor.val[3];      
    
    matOut.at<cv::Vec4b>(y+yOrigin, x+xOrigin) = cv::Vec4b(blue, green, red, alpha);
  }
}
...
makeFourChanImage(const cv::Mat &src)
{
  ...
  cv::Mat dst = cv::Mat(src.rows, src.cols, CV_8UC4); //convert 3 channels mat in 4 channels mat
...
cv::Mat
holeDegradation(const cv::Mat &matOriginal, const cv::Mat &pattern, 
		int xOrigin, int yOrigin, int size, HoleType type, int side, QColor color, const cv::Mat &matBelow, int width, float intensity)
{
  ...
  if (size != 0 && matPattern.cols+size > 0 && matPattern.rows+size > 0)
      resize(matPattern, matPattern, cv::Size(matPattern.cols+size, matPattern.rows+size));
  ...
  const cv::Vec4b color4(color.blue(), color.green(), color.red(), color.alpha());
  const cv::Vec4b white4(255, 255, 255, 255); //B:TODO: pass the below color as parameter
  ...
  if (matBelow.empty()) { //without below image 
    ...
    for (int y=y0; y<y1; ++y) {
      ...
      for (int x=x0; x<x1; ++x) {
	assert(x+xOrigin >= 0 && x+xOrigin < matOut.cols && y+yOrigin >=0 && y+yOrigin < matOut.rows);
	if (p[x] == PIXEL_BLACK) 
	  d[x+xOrigin] = color4;
      }
    }
	...
	if (p[x] == PIXEL_BLACK) {
	  if (b != nullptr && x+xOrigin>=0 && x+xOrigin < matBelow.cols) {
	    ...
	    d[x+xOrigin] = white4; //if page below is not below this pixel because of its size, we draw a white pixel to represent what we can see when it arrive with scanner
	  }

	  drawBorder(matOut, matPattern, x, y, xOrigin, yOrigin, width, intensity);
	}
      }
    }
...
QImage holeDegradation(const QImage &imgOriginal, const QImage &pattern, 
		       int xOrigin, int yOrigin, int size, HoleType type, int side, 
		       QColor color, const QImage &pageBelow, int width, float intensity)
{
  ...
  cv::Mat matOut = holeDegradation(matOriginal, matPattern, xOrigin, yOrigin, size, type, side, color, matBelow, width, intensity);
